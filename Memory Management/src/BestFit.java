...
public class BestFit {
    ...
    BestFit(Process[] proc,Partition[] part)
    {
    ...
    private void PutProcessInMemory()
    {
        for(int i=0;i<=processes.length-1;i++)
        {
            ...
            int place = FindBest(processes[i].size);
            if(place >=0)
            {
    ...
    private void PutOutProcessInMemory()
    {
        ...
        {
            //find the best place for the process
            int place = FindBest(out_processes.get(i).size);
            if(place >=0)
            {
                int old_size = partitions_for_print.get(place).size;
                //put the process in the best partition
    ...
    private int FindBest (int s)
    {
        ...
        for(int i=0;i<partitions_for_print.size();i++)
        {
            if(min_size != -1)
            {
                if(partitions_for_print.get(i).size>=s && partitions_for_print.get(i).size<min_size && partitions_for_print.get(i).process_name == "External fragment")
                {
                    place = i;
                    min_size = partitions_for_print.get(i).size;
                }
            }else
            {
